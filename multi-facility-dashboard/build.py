...
MAINT_LOG_NAME = "maintenance_log.xlsx"
...
def read_maintenance_log(path):
    """Read a maintenance_log.xlsx → ({device_code: [event, ...]}, {device_code: support}).
    ...
    ws = openpyxl.load_workbook(path, data_only=True)["Log"]
...
def build_facility(data_dir, config, default_life):
    """Read every category .xlsx directly under data_dir into a {category_key: {...}} dict."""
    log_events, log_support = read_maintenance_log(data_dir / MAINT_LOG_NAME)
    categories = {}
    for path in sorted(data_dir.glob("*.xlsx")):
        if path.name == MAINT_LOG_NAME or path.name.startswith("~$"):
            continue
        key = path.stem.lower()
...
def main():
    ...
    facility_cfg = config.get("facilities", {"main": {"name_en": "Main", "name_ar": "الرئيسي"}})

    facilities = {}
    for key, meta in facility_cfg.items():
        data_dir = DATA_DIR if key == "main" else FACILITIES_DIR / key
        if not data_dir.is_dir():
            print(f"WARNING: facility '{key}' has no data directory ({data_dir}), skipping")
            continue
        print(f"-- facility: {key} --")
        facilities[key] = {**meta, "categories": build_facility(data_dir, config, default_life)}

    for entry in sorted(FACILITIES_DIR.glob("*")) if FACILITIES_DIR.is_dir() else []:
        if entry.is_dir() and entry.name not in facility_cfg:
            print(f"WARNING: data/facilities/{entry.name} has no config.json facilities entry, skipping")
